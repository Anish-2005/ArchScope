...
export const ReportCard = ({ data }: { data: StackReport }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            {/* Header Card */}
            <div className="bg-white/[0.07] border border-white/15 rounded-3xl p-6 sm:p-10 relative overflow-hidden backdrop-blur-2xl shadow-[0_24px_64px_rgba(2,8,20,0.35)]">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    <div className="space-y-4 max-w-2xl">
                        ...
                        <div className="flex flex-wrap items-center gap-4 pt-2 text-sm text-zinc-300 font-medium">
            ...
            <div className="bg-white/[0.06] border border-white/14 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-[0_16px_42px_rgba(2,8,20,0.28)]">
...
const CategoryBlock = ({ title, icon, items, emptyText, isCompact = false }: { title: string; icon: React.ReactNode; items: string[]; emptyText: string; isCompact?: boolean }) => {
    return (
        <div className="bg-white/[0.05] border border-white/12 rounded-2xl p-6 hover:bg-white/[0.1] hover:border-white/20 transition-all flex flex-col h-full shadow-[0_14px_36px_rgba(2,8,20,0.22)] backdrop-blur-xl">
                    ...
                    <div className={`flex flex-wrap gap-2 ${isCompact ? 'gap-y-2' : 'gap-y-2'}`}>
                        {items.map((item, i) => (
                            <motion.span
                                ...
                                key={item}
                                className="px-3 py-1.5 bg-white/10 text-zinc-100 font-mono text-xs rounded-lg border border-white/15 shadow-sm whitespace-nowrap"
                            >
                                {item}
                            </motion.span>
                        ))}
...
const ComplexityGauge = ({ score }: { score: number }) => {
    ...
    return (
        <div className={`flex flex-col items-center justify-center p-6 sm:p-8 bg-white/[0.08] rounded-3xl ring-1 ${bgRing} shadow-[0_18px_42px_rgba(2,8,20,0.32)] relative shrink-0 min-w-[200px] backdrop-blur-xl border border-white/12`}>
            <span className="text-zinc-200 text-xs sm:text-sm uppercase font-mono tracking-widest mb-4">Architecture Index</span>
            <div className={`w-32 h-32 rounded-full border border-white/12 border-t-[4px] ${borderColor} flex flex-col items-center justify-center bg-slate-950/30 shadow-inner ring-4 ring-slate-900/25`}>
